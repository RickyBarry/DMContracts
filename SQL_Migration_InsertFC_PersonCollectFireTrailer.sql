...
/***	Where the FC_TYPE = Person to Collect Fire trailer			***/
...
SELECT	fst.OBJECTID,
  ...
  WHERE FC_TYPE = 'Person to collect Fire Trailer'    -- Just the Person to Collect Fire Trailer
...
'sde.DEFAULT', 'mvInsert_PersonFireTrailer', 1, 1, 'multiversioned view insert - Forest Internal Contact'

EXEC SDE.SDE.set_current_version 'mvInsert_PersonFireTrailer'

EXEC SDE.SDE.edit_version 'mvInsert_PersonFireTrailer', 1
...
INSERT INTO [tfm].[dbo].[VMV_TFM_CMN_FOREST_CONTACTS]
	...
	SELECT
		...
		NULL	AS PartyID,
		FC_HRID,
		'PCFT',				-- Hard coded for Person to Collect Fire Trailer
		ForestCode,
		'' AS Comments,
...
EXEC SDE.SDE.edit_version 'mvInsert_PersonFireTrailer', 2

 DROP TABLE #ForestContactsInternal

-- Test
--SELECT * FROM VMV_TFM_CMN_FOREST_CONTACTS
--  WHERE [FOREST_CONTACTS_ROLE] = 'PCFT'
