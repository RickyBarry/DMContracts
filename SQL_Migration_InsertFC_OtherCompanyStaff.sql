...
/***	Where the FC_TYPE = Other Company Staff						***/
...
SELECT	fst.OBJECTID,
  ...
  WHERE FC_TYPE = 'Other Company Staff'    -- Just the other company staff
...
'sde.DEFAULT', 'mvInsert_OtherCompanyStaff', 1, 1, 'multiversioned view insert - Forest Internal Contact'

EXEC SDE.SDE.set_current_version 'mvInsert_OtherCompanyStaff'

EXEC SDE.SDE.edit_version 'mvInsert_OtherCompanyStaff', 1
...
INSERT INTO [tfm].[dbo].[VMV_TFM_CMN_FOREST_CONTACTS]
	...
	SELECT
		...
		NULL	AS PartyID,
		FC_HRID,
		FCRole,					-- Hard coded for Other Company Staff
		ForestCode,
		'' AS Comments,
...
EXEC SDE.SDE.edit_version 'mvInsert_OtherCompanyStaff', 2
...
--  WHERE [FOREST_CONTACTS_ROLE] = 'OCS'
