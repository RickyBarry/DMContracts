...
SELECT	fst.OBJECTID,
  ...
  WHERE FC_TYPE = 'CRA Holder'    -- Just the CRA Holders
...
'sde.DEFAULT', 'mvInsert_ForestCRAHolder', 1, 1, 'multiversioned view insert - Forest Internal Contact'

EXEC SDE.SDE.set_current_version 'mvInsert_ForestCRAHolder'

EXEC SDE.SDE.edit_version 'mvInsert_ForestCRAHolder', 1
...
INSERT INTO [tfm].[dbo].[VMV_TFM_CMN_FOREST_CONTACTS]
	...
	SELECT
		...
		NULL	AS PartyID,
		FC_HRID,
		FCRole,					-- Hard coded for CRA Holders
		ForestCode,
		'' AS Comments,
...
EXEC SDE.SDE.edit_version 'mvInsert_ForestCRAHolder', 2

DROP TABLE #ForestContactsInternal

-- Test
--SELECT * FROM VMV_TFM_CMN_FOREST_CONTACTS
--  WHERE [FOREST_CONTACTS_ROLE] = 'CRAH'
