...
SELECT	fst.OBJECTID,
  ...
  WHERE FC_TYPE = 'Caretakers'    -- Just the Caretakers
...
'sde.DEFAULT', 'mvInsert_ForestCaretaker', 1, 1, 'multiversioned view insert - Forest Internal Contact'

EXEC SDE.SDE.set_current_version 'mvInsert_ForestCaretaker'

EXEC SDE.SDE.edit_version 'mvInsert_ForestCaretaker', 1
...
INSERT INTO [tfm].[dbo].[VMV_TFM_CMN_FOREST_CONTACTS]
	...
	SELECT
		...
		NULL	AS PartyID,
		FC_HRID,
		FCRole,				-- Hard coded for Caretaker
		ForestCode,
		'' AS Comments,
...
EXEC SDE.SDE.edit_version 'mvInsert_ForestCaretaker', 2

DROP TABLE #ForestContactsInternal

-- Test
--SELECT * FROM VMV_TFM_CMN_FOREST_CONTACTS
--  WHERE [FOREST_CONTACTS_ROLE] = 'CARE'
