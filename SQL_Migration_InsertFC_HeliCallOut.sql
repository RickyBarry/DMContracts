...
/***	Where the FC_TYPE = Helicopter Call-Out Personnel			***/
...
SELECT	fst.OBJECTID,
  ...
  WHERE FC_TYPE = 'Helicopter Call-Out Personnel'    -- Just the Helicopter call out 
...
'sde.DEFAULT', 'mvInsert_HeliCallout', 1, 1, 'multiversioned view insert - Forest Internal Contact'

EXEC SDE.SDE.set_current_version 'mvInsert_HeliCallout'

EXEC SDE.SDE.edit_version 'mvInsert_HeliCallout', 1
...
INSERT INTO [tfm].[dbo].[VMV_TFM_CMN_FOREST_CONTACTS]
	...
	SELECT
		...
		NULL	AS PartyID,
		FC_HRID,
		FCRole,				-- Hard Coded for Helicopter Call Out
		ForestCode,
		'' AS Comments,
...
EXEC SDE.SDE.edit_version 'mvInsert_HeliCallout', 2
...
--  WHERE [FOREST_CONTACTS_ROLE] = 'HELIP'
